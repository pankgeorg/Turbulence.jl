...
| 3 | SA implemented + channel395 RANS check            | Layer 2 ±5%                                            | ✅ closure (κ≈0.40, physical ν_t, L1); channel log-law 17%→**7.4% mean** with the BDIM wall function (<2% outer layer). `ShipFlow.jl/RESULTS-channel-sa.md` |
| 4 | k–ω SST implemented + channel395 + backstep       | Layer 2 + Layer 3 pass                                 | ✅ channel 1.7% (native ω-wall); **Layer-3 BFS** vs OF kΩSST on identical ER=2 step (OF 8.35, WL-SST 9.90 +19%, WL-SA 8.50 +2%) — bubble reproduced, SST over-predicts (BDIM shear-layer diffusion). `ShipFlow.jl/RESULTS-bfs-sst.md` |
| — | **BDIM wall function** (Spalding ν_t override)    | SA channel log-law within ±10%                        | ✅ mean gate met (7.4%). SA-only — SST's native ω-wall handles BDIM. Refinement: taper the band override to remove the buffer-band artifact |
